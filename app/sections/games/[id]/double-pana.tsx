...
const DOUBLE_PANA_URL = 'https://api.rmgames.live/api/user-bets/doublepanadigitsbets'
...
const DOUBLE_PANA_NUMBERS = [
  ...
  '113', '122', '177', '339', '447', '500', '799', '889',
...
export default function DoublePanaScreen() {
  ...
  const [filteredNumbers, setFilteredNumbers] = useState<string[]>(DOUBLE_PANA_NUMBERS)
  ...
  useEffect(() => {
    const title = gameName ? `${gameName} - Double Pana` : 'Double Pana'
      ...
      .onSnapshot(
        (snap) => {
          ...
          if (!canOpen && !canClose && !initialCheckDone) {
            ...
            Alert.alert('Not Available', 'Double Pana betting is closed for this game.')
            navigation.goBack()
          }
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        console.time('DoublePana warmup')
        ...
        if (!abort) console.timeEnd('DoublePana warmup')
      } catch (err) {
        try { console.timeEnd('DoublePana warmup') } catch (_) {}
      }
    })()
    return () => { abort = true }
  }, [initialCheckDone, id])
  ...
  const onFilterNumbers = (text: string) => {
    ...
    if (text.length === 3 && !DOUBLE_PANA_NUMBERS.includes(text)) {
      setNumber('')
      setFilteredNumbers(DOUBLE_PANA_NUMBERS)
    ...
    let next: string[]
    if (text.length === 0) {
      next = DOUBLE_PANA_NUMBERS
    } else {
      next = DOUBLE_PANA_NUMBERS.filter((n) => n.startsWith(text))
    }
  ...
  const onAdd = () => {
    ...
    if (!DOUBLE_PANA_NUMBERS.includes(number)) {
      Alert.alert('Invalid Number', 'Please select a valid double pana number from the dropdown.')
      return
    }
  ...
  const onPressSubmit = () => {
    ...
    // Check wallet sufficiency before attempting submission
  ...
  const performSubmit = async () => {
    ...
    try {
      ...
      const resp = await fetch(DOUBLE_PANA_URL, {
        method: 'POST',
    ...
    return (
      <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', padding: 16 }}>
        <Text style={{ fontSize: 16, fontWeight: '600', color: '#ef4444', textAlign: 'center' }}>
          Double Pana betting is closed for this game.
        </Text>
      </View>
    )
  }
...
function triggerChartRequest(payload: SubmitPayload, token: string | null) {
  // Fire-and-forget chart sync so primary submit stays responsive
  ;(async () => {
    try {

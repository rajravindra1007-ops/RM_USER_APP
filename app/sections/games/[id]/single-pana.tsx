...
export default function SinglePanaScreen() {
  ...
  const [filteredNumbers, setFilteredNumbers] = useState<string[]>(PANA_NUMBERS)
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        if (!abort) console.timeEnd('SP-page warmup')
      } catch (err) {
        try { console.timeEnd('SP-page warmup') } catch (_) {}
      }
    })()
    return () => { abort = true }
  }, [initialCheckDone, id])
  ...
  const onFilterNumbers = (text: string) => {
    ...
    if (text.length === 3 && !PANA_NUMBERS.includes(text)) {
      setNumber('')
      setFilteredNumbers(PANA_NUMBERS)
  ...
  useEffect(() => {
    // ensure dropdown + focus when switching between open/close
    try {
  ...
  const performSubmit = async () => {
    ...
    try {
      ...
      const resp = await fetch(SINGLE_PANA_URL, {
        method: 'POST',
    ...
    return (
      <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', padding: 16 }}>
        <Text style={{ fontSize: 16, fontWeight: '600', color: '#ef4444', textAlign: 'center' }}>
          Single Pana betting is closed for this game.
        </Text>
      </View>
    )
  }
...
function triggerChartRequest(payload: SubmitPayload, token: string | null) {
  // Fire-and-forget to keep main submit responsive
  ;(async () => {
    try {
